...
namespace PlatformWeb.Controller
{
    ...
    public class DockCollectionReportsController : ApiController
    {
        ...
        public IHttpActionResult GetDockCollectionSummaryByVLC(int id, [FromUri] DateTime collectionStartDate, [FromUri] DateTime collectionEndDate)
        {
            try
            {
                return Ok(_dockCollectionReportService.DockCollectionSummaryByVLC(id, collectionStartDate, collectionEndDate));
            }
            catch (PlatformModuleException ex)
            {
                return Ok(ResponseHelper.CreateResponseDTOForException(ex.Message));
