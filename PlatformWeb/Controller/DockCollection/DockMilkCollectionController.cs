...
namespace PlatformWeb.Controller
{
    ...
    public class DockMilkCollectionsController : ApiController
    {
        ...
        [HttpPost]
        public IHttpActionResult Get([FromUri] int dockId, [FromUri]DateTime collectionDate, [FromUri] int shift, [FromUri] int pageNumber)
        {
            try
            {
                ResponseDTO responseDTO = _dockMilkCollectionService.GetDockMilkCollectionsByDateAndShift(dockId, collectionDate, shift, pageNumber);
